...
namespace WindowsFormsApp3
{
    public partial class Form2 : Form
    {
        ...
        {
            StreamReader file = new StreamReader(date.Month.ToString() + "月入退出記録.csv");
            file.ReadLine();
            ...
            while (!file_r.EndOfStream)
            {
                ...
                try
                {
                    ...
                    if (result.Text == vals[0])
                    {
                        ...
                        // ヘッダ部につき、コメントアウト。
                        file_w.WriteLine(string.Format("{0},{1},入室,{2}", vals[0], vals[1], date.ToShortDateString() + "  " + date.ToLongTimeString()));
                        timer1.Stop();
                        file_w.Close();
                        capture.Dispose();
                        MessageBox.Show(string.Format("{0}さんこんにちは！", vals[1]));
